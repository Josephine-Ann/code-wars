function tribonacci(signature,n){
    if (n === 0) {
        return 0
    } else {
    const reducer = (accumulator, currentValue) => accumulator + currentValue;
    let signatureTwo = []
    signatureTwo = signatureTwo.concat(signature);
       for (let sequence = 0; sequence < n; sequence++) {
         let lastThree = signature.splice(signature.length-3, signature.length)
         let toAdd = lastThree.reduce(reducer)
         signatureTwo = signatureTwo.concat(toAdd)
         signature = signature.concat(signatureTwo)
         signatureTwo = signatureTwo.slice(0, n)
     }
    return signatureTwo
    }
 }
 console.log(tribonacci([0.5,0.5,0.5],30))
//  Test.assertSimilar(tribonacci([1,1,1],1),[1])
//  Test.assertSimilar(tribonacci([300,200,100],0),[])
//  Test.assertSimilar(tribonacci([0.5,0.5,0.5],30),[0.5,0.5,0.5,1.5,2.5,4.5,8.5,15.5,28.5,52.5,96.5,177.5,326.5,600.5,1104.5,2031.5,3736.5,6872.5,12640.5,23249.5,42762.5,78652.5,144664.5,266079.5,489396.5,900140.5,1655616.5,3045153.5,5600910.5,10301680.5])